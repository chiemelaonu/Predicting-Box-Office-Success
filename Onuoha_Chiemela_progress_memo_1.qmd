...
## Prediction Problem
The goal of this study is to predict pancreatic cancer survival based on demographic, lifestyle, and medical history variables. This is a binary classification problem, where the target variable is Survival_Status (0 = Not Survived, 1 = Survived).  
<br>  
...
Symmetrical distributions are balanced around the mean, so the model doesn't get influenced by outliers or skewed data. There are also substantial amount of missing values in the data set, so removing the rows where there are missing observations would be helpful. Since there are so many missing data points, I plan on only removing the rows that a missing observation in the feature columns I plan on using for the model.