...
:::
On the left, we have the original distribution of the target variable, visualized with a density plot and box plot. The original values were heavily skewed right, so it was apparent a transformation of revenue was needed. After experimenting with log, Yeo-Johnson, square root, and Box-Cox transformations, I settled on a Yeo-Johnson transformation to make the values more evenly distributed. The density and box plot on the right shows those values post-transformation. I selected a lambda value of 0.25 for the transformation because it gave the most even distribution.